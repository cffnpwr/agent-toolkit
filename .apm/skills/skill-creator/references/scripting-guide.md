...
# Node.js — run inside the pnpm-managed environment
pnpm exec node scripts/extract.js
pnpm run extract
```
...
依存パッケージはコミット済みロックファイルから同期する（`uv sync --frozen`・`pnpm install --frozen-lockfile` 等）。これは `.venv`・`node_modules` 等のgit管理外のみを変更しgit検知の更新を生まないため、エージェントが自律実行してよい。一方、外部ツールの導入、依存宣言・ロックファイルの変更（依存の追加・更新）、代替手段へのフォールバックは行わず、停止してユーザーへエスカレーションする。依存宣言の変更はスキルの実行前に行う作業であり、タスクの途中で即興で行うものではない。
