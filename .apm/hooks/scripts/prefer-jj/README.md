...
- 出力は移植性の高い終了コードに一本化する。
  ブロックはexit 2 + stderr、実行不可(fail-open)はexit 1 + stderr、通過・対象外はexit 0・無出力。
...
| 状況 | 出力 | 効果 |
| --- | --- | --- |
| ブロック(denylistのサブコマンド、フラグ例外に該当しない) | exit 2 + stderr(サブコマンド別のjj相当を提示) | Claude/Codex/Geminiにフィードバック |
| 通過(非git / git専用 / フラグ例外 / 非jjリポジトリ / 無効化) | exit 0・無出力 | 何もしない |
| 実行不可(fail-open。bun不在 / 依存同期失敗 / 想定外の例外) | exit 1 + stderr | Claude/Codex/Geminiで非ブロック警告 |
...
`apm install`は依存パッケージ本体(node_modules)を導入しないため、`package.json`・`bun.lock`もhook定義(`.apm/hooks/prefer-jj.json`)のcommandに列挙してコピー対象に含め、ホスト側の同期コマンドで導入する。
