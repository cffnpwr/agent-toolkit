...
if ! command -v bun >/dev/null 2>&1; then
  echo "approval-chain-guard: bun not found; cannot check for chained commands." >&2
  exit 1
fi
...
if ! ( cd "$dir" && bun "$dir/../shared/src/gen-package-json.ts" "$dir" && bun install --frozen-lockfile --production --ignore-scripts ) >/dev/null 2>&1; then
  echo "approval-chain-guard: failed to sync the bundled deps from the lockfile; cannot check for chained commands." >&2
  exit 1
fi
