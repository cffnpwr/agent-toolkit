...
describe("runCommitlint", () => {
  ...
  it("[positive] リポジトリ設定が無ければ同梱デフォルトを使い、breaking change記法(!)は違反にしない", async () => {
  ...
  it("[negative] 同梱デフォルトで許可されていないtypeは違反として報告する", async () => {
    const result = await runCommitlint("unknown: :sparkles: do something", noConfigDir);
    expect(result.ok).toBe(false);
    expect(result.unavailable).toBe(false);
  });

  it("[positive] リポジトリ設定があればそれを優先し、設定内のtypeは通過させる", async () => {
    const result = await runCommitlint("custom: do something", repoDir);
    expect(result).toEqual({ ok: true, report: "", unavailable: false });
  });

  it("[negative] リポジトリ設定を優先し、同梱デフォルトでは通るtypeでもenum外なら違反にする", async () => {
