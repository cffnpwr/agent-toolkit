...
// PostToolUse入力からシェルコマンド文字列を取り出す。
// shell系ツールのtool_inputは{ command: string }。
// 出典: Claude(hooks docs, Bashツール)、Codex(codex-rs/hooks/src/events/post_tool_use.rs)、Gemini(packages/core/src/tools/shell.ts ShellToolParams.command)。
